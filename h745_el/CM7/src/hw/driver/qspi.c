...
#define QSPI_BK1_CS_GPIO_AF        GPIO_AF9_QUADSPI
...
bool qspiInit(void)
{
  ...
  if (BSP_QSPI_GetID(&info) == QSPI_OK)
  {
    ...
    {
      logPrintf("MT25QL512A(QSPI) \t: OK\r\n");
      ret = true;
    }
    else
    {
      logPrintf("MT25QL512A(QSPI) \t : Fail\r\n");
      ret = false;
    }
  }
  else
...
bool qspiErase(uint32_t addr, uint32_t length)
{
  ...
  flash_length = N25Q512A_FLASH_SIZE;
  block_size   = N25Q512A_SUBSECTOR_SIZE;
...
uint8_t BSP_QSPI_Erase_Sector(uint32_t SectorAddress)
{
  ...
  /* Configure automatic polling mode to wait for end of erase */
  if (QSPI_AutoPollingMemReady(&QSPIHandle, N25Q512A_SECTOR_ERASE_MAX_TIME) != QSPI_OK)
  {
    return QSPI_ERROR;
  }
...
static uint8_t QSPI_EnterFourBytesAddress(QSPI_HandleTypeDef *hqspi)
{
  ...
  {
    logPrintf("QSPI_WriteEnable() fail\n");
    return QSPI_ERROR;
  }
  ...
  if (QSPI_AutoPollingMemReady(hqspi, HAL_QPSI_TIMEOUT_DEFAULT_VALUE) != QSPI_OK)
  {
    logPrintf("QSPI_AutoPollingMemReady() fail\n");
    return QSPI_ERROR;
  }
